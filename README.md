...
3. `1.3` 本文贡献，通常分为两部分，系统实现细节和主要贡献：
   ...
   3. 贡献需要体现技术点和工作内容，不能只谈技术点
4. `1.4` 文章其他部分介绍，格式注意统一。
...
3. 对于方法类论文，介绍时要体现其他方法存在的问题，以凸显自身技术的优势
...
如果是方法类论文，本章为方法介绍，需要注意：
1. 章节名尽量不要和论文题目重合
2. 如果概述介绍了方法的几个部分，接下来就分为几个小节进行独立介绍，顺序保持一致
3. 每个小节最好在开头概述该部分的作用、技术点，以及输入输出部分
4. 每小节内容介绍最好搭配伪代码算法或图表行介绍
...
如果是方法类论文，本章为实验设计和结果分析，参考第五部分的技术评估部分
1. 需要介绍本文使用的数据集、实验环境、模型、评估指标、基线、研究问题等
2. 结果分析中，需要对每个研究问题展开进行细致分析，结合图、表、示例进行说明
3. 舍得放图表，内容多没关系，能够凸显工作量
...
如果是方法类论文，本章一般为基于第三章方法设计并实现的一个系统，可参考系统类论文三、四章内容进行撰写
...
引用中不要引用本硕博学位论文！
文章中需要有 5 篇及以上的中文 CCF-A 参考文献，推荐计算机学报、软件学报等期刊。
